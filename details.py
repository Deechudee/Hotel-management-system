...
class DetailsRoom:
    def __init__(self,root):
        ...
        lblframeleft = LabelFrame(self.root,bd=2,relief=RIDGE, text="New Room Add",font=("arial",12,"bold"),padx=2 )
        lblframeleft.place(x=5, y=50, width=540, height=350)
 
 # ##### Floor
        lbl_floor= Label(lblframeleft, text="Floor",font=("arial",12,"bold"), padx=2,pady=6)
        lbl_floor.grid(row=0,column=0,sticky=W,padx=20)

        self.var_floor=StringVar()
        enty_floor= ttk.Entry(lblframeleft,textvariable=self.var_floor,font=("arial",13,"bold"), width=20)
        enty_floor.grid(row=0,column=1,sticky=W)

        # Room No
        lbl_RoomNo= Label(lblframeleft, text="Room No",font=("arial",12,"bold"), padx=2,pady=6)
        lbl_RoomNo.grid(row=1,column=0,sticky=W,padx=20)

        self.var_roomNo=StringVar()
        enty_RoomNo= ttk.Entry(lblframeleft,textvariable=self.var_roomNo,font=("arial",13,"bold"), width=20)
        enty_RoomNo.grid(row=1,column=1,sticky=W)

        #Room type
        lbl_RoomType= Label(lblframeleft, text="Room Type",font=("arial",12,"bold"), padx=2,pady=6)
        lbl_RoomType.grid(row=2,column=0,sticky=W,padx=20)

        self.var_RoomType=StringVar()
        enty_RoomType= ttk.Entry(lblframeleft,textvariable=self.var_RoomType,font=("arial",13,"bold"), width=20)
        ...
        btnReset=Button(btn_frame,text="Reset",command=self.reset_details,font=("arial",11,"bold"),bg="black",fg="gold",width=10)
  ...
  # Fetch data
    def fetch_data(self):
        conn = mysql.connector.connect(host="localhost", username="root", password="deeChu@2004", database="hotel_db")
        my_cursor = conn.cursor()
        my_cursor.execute("select * from details")  # Changed from room to details
